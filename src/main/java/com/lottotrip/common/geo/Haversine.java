 ...
 *
 * <p><b>왜 우리가 직접 계산하나.</b> 결정 8(온디맨드) 시절에는 TourAPI의
 * {@code locationBasedList2}가 {@code dist}(요청 좌표로부터의 거리)를 함께 줬다.
 * 결정 10으로 추첨이 DB 조회로 바뀌면서 받아올 응답 자체가 없어졌고,
 * <b>거리 계산이 우리 몫이 되었다.</b> 슬롯 응답의 {@code distanceKm}가 여기서 나온다.
 *
 * <p><b>왜 피타고라스로 하지 않나.</b> 위도·경도는 평면 좌표가 아니다. 경도 1도의 실제 거리는
 * 적도에서 약 111km지만 강원(위도 37.75)에서는 약 88km다. 두 값을 그냥 제곱해 더하면
 * 동서 거리가 20% 넘게 부풀려진다. 반경 10km 판정에서 이 차이는 그대로 오차가 된다.
 *
 * <p>계산 자체는 무겁지 않지만 <b>삼각함수라 DB 인덱스를 타지 못한다.</b> 그래서 이 계산을
 * 바로 쓰지 않고 {@link BoundingBox}로 후보를 좁힌 뒤 2차 검증으로만 쓴다.
 *
 * <p>8-1(미션 GPS 인증)도 "정해진 반경 안에 있는가"를 판단해야 하므로 이 클래스를 함께 쓴다.
 * 도메인에 매이지 않은 순수 계산이라 {@code common}에 둔다.
 */
public final class Haversine {
     ...
     * 지구 반지름(km). 평균값이다.
     *
     * <p>지구는 완전한 구가 아니라 적도 쪽이 조금 부풀어 있다(적도 6,378km / 극 6,357km).
     ...
     * 두 좌표 사이의 거리(km).
     *
     * <p>구 위의 두 점을 잇는 <b>가장 짧은 곡선(대권거리)</b>을 잰다. 지표를 따라 재므로
     * 사람이 이동하는 거리 감각과 맞는다.
     *
     * <p>{@code asin}을 쓰는 형태를 택한 이유: 두 점이 아주 가까울 때 흔히 쓰는
     * {@code acos} 형태는 부동소수점 오차로 값이 뭉개져 거리가 통째로 0이 되기도 한다.
