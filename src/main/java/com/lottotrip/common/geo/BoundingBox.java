 ...
 *
 * **역할은 1차 필터다.** {@link Haversine}은 삼각함수라 DB 인덱스를 타지 못해,
 * 바로 계산하면 강원 전체(2,373건)를 매번 훑는다. 사각형은 단순 부등호 비교라
 * `idx_places_coordinate`를 태울 수 있다. 수십 건으로 줄인 뒤 정확한 거리를 재는 편이 훨씬 싸다.
 *
 * ## 넉넉한 쪽으로 틀려야 한다
 * 이 사각형은 **원을 완전히 감싸야** 한다. 조금이라도 작으면 반경 안에 있는 장소가
 * 1차 필터에서 탈락해 **영영 뽑히지 않는다.** 반대로 원 밖이 조금 딸려 오는 것은
 * 2차 Haversine이 걸러 주므로 아무 문제가 없다. 그래서 각 방향의 폭을 계산할 때
 * **거리가 가장 짧게 잡히는 값**을 분모로 써서 각도를 크게 만든다.
 *
...
public record BoundingBox(
     ...
     *
     * 지구가 완전한 구가 아니라 위도 1도는 적도 부근 약 110.57km, 극 부근 약 111.69km로
     ...
     *
     * 극에 다가가면 `cos(위도)`가 0에 수렴해 경도 폭이 무한대로 발산한다.
     * 그대로 두면 조회 조건이 NaN이 되어 **결과가 조용히 항상 비게 된다.**
