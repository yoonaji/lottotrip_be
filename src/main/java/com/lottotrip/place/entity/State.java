...
public class State {
    ...
    /**
     * TourAPI 시도 코드. 강원 = {@code "32"}, 서울 = {@code "1"}. (roadmap 5-8, 결정 10)
     *
     * <p>TourAPI는 장소의 지역을 <b>코드로만</b> 준다(이름은 주지 않는다). 이 코드를 담아 두어야
     * 장소를 적재할 때 어느 지역 행에 붙일지 정할 수 있다.
     *
     * <p><b>숫자가 아니라 문자열인 이유:</b> 우리가 계산에 쓰는 값이 아니라 API가 준 식별자를
     * 그대로 보관하는 것이다. 앞자리 0이 붙거나 숫자가 아닌 코드가 생겨도 그대로 담긴다.
     *
     * <p>nullable인 이유는 시드 전에도 지역 행을 만들 수 있어야 하기 때문이다(테스트 픽스처 등).
     */
    ...
    /**
     * 지역명을 갱신한다. 시드가 <b>코드가 같은 기존 행을 찾았을 때</b>만 쓴다.
     *
     * <p>TourAPI의 지역명은 바뀔 수 있다("강원" → "강원특별자치도"). 코드가 같으면 같은 지역이므로
     * 행을 새로 만들지 않고 이름만 맞춘다. 새로 만들면 {@code places.city_id}가 어느 쪽을 가리켜야 할지
     * 알 수 없게 된다.
     *
     * <p>바꿀 수 있는 것은 이름뿐이다. {@code tourAreaCode}는 이 행이 무엇인지 정하는 값이라
     * 바뀌면 다른 지역이 되어 버린다.
     */
