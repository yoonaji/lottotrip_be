...
public record TourApiPlaceItem(
     ...
     *
     * 정기 갱신 배치가 **무엇을 다시 받을지** 고르는 기준이다. 목록 조회는 싸고(14회)
     ...
     *
     * ⚠️ **슬롯 추첨은 이 값을 쓰지 않는다.** 결정 10으로 추첨이 DB 조회로 바뀌어
     * 받아올 응답 자체가 없다. 추첨의 `distanceKm`는 우리가 Haversine으로 직접 계산한다.
     *
     ...
     *
     * 수만 건을 훑는 배치에서 한 건이 깨졌다고 전체가 멈추면 안 된다.
