 ...
 *
 * <p>이 API는 어떤 오퍼레이션이든 아래 구조를 그대로 쓴다. 안에 담기는 <b>항목의 종류만</b> 다르다.
 *
 * <pre>
 ...
 * </pre>
 *
 * <p>그래서 {@code <T>}(제네릭)로 만들었다. 제네릭은 "담기는 것의 종류를 나중에 정하는 상자"다.
 * 상자를 종류마다 새로 만들지 않아도 {@code TourApiResponse<TourApiPlaceItem>},
 * {@code TourApiResponse<TourApiImageItem>}처럼 필요한 만큼 찍어 쓸 수 있다.
 *
 * <p>⚠️ <b>{@code resultCode}는 HTTP 상태와 별개다.</b> 이 API는 인증 실패·할당량 초과에도
...
public record TourApiResponse<T>(Response<T> response) {
    ...
    /**
     * 항목 목록. 어느 단계가 비어 있어도 <b>빈 리스트</b>를 돌려준다.
     *
     * <p>{@code null}을 돌려주면 쓰는 쪽이 매번 null 검사를 해야 하고, 한 곳만 빠뜨려도
     * {@code NullPointerException}이 난다. "없음"은 null이 아니라 빈 리스트로 표현하는 편이 안전하다.
     *
     * <p>결과가 0건일 때 {@code "items": ""}(빈 문자열)로 오는 경우가 있는데,
