 ...
 * **영원히 참**이 된다. 2026-08-14에 실제로 이 때문에 배치가 무한 루프에 빠져
...
public record TourApiPage<T>(List<T> items, int pageNo, int numOfRows, int totalCount) {
     ...
     * 결정 12로 `draw`가 `numOfRows=1000` 1회 호출로 고정되면서 페이지 순회 자체가 없어졌다.
     * **남겨 둔 이유:** 1,000건 상한에 걸리는 좌표(수도권)를 나중에 살리려면 2번째 호출이 필요해진다.
     * 지울지 여부는 사용자에게 확인한다.
     *
     * **두 겹으로 막는다.** 어느 한쪽이 뚫려도 순회가 폭주하지 않아야 한다.
     *
