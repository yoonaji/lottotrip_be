 ...
 *
 * `@ConfigurationProperties`는 `application.yml`의 `tourapi.*` 값을
 ...
 * @param withBaseUrl 무장애 여행 정보(`KorWithService2`) 주소. 인증키는 같은 것을 쓴다
 * @param serviceKey  공공데이터포털 인증키. **디코딩(Decoding) 값**을 넣는다.
 *                    인코딩 값을 넣으면 클라이언트가 한 번 더 인코딩해 이중 인코딩이 된다.
 * @param numOfRows   한 번에 받아올 건수. 크게 잡을수록 호출 횟수가 준다. **API 상한은 1,000이다**
 */
