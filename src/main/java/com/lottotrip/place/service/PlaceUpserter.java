...
/**
 * 뽑힌 장소 하나를 `places`테이블에 담는다. (roadmap 6-12, 결정 12)
 *
 * 추첨된 1건만 저장하므로, 여기서 실패하면 그 장소는 사용자에게 나가지 못한다.
 *
 * 왜 저장하는가: 슬롯 결과(`saved_slots`)와 미션(`missions`)이 모두
 * `place_id`를 가리킨다. 뽑기만 하고 담지 않으면 이어 붙일 곳이 없다.
 * 나중에 결과를 다시 조회할 때 `content_id`로 실시간 세부 정보도 받아 와야 한다.
 *
 * 여러 번 뽑혀도 안전하다. `content_id`로 찾아 있으면 갱신한다.(UpSert)
 */
@Slf4j
@Component
...
public class PlaceUpserter {
     ...
     *
     * 좌표 기반으로 조회해 받은 결과이므로 좌표가 없을 수 없다 — 정상적이라면 실사용 없음
     * 그런데도 여기 왔다면 걸러야 할 것을 놓친 우리 코드의 버그다.
     */
