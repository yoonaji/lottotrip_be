...
/**
 * 뽑힌 장소 하나를 `places`에 담는다. (roadmap 6-12, 결정 12)
 *
 * 배치가 강원 전역을 미리 담던 자리를 대신한다. 온디맨드에서는 **추첨된 1건만** 저장하므로,
 * 여기서 실패하면 그 장소는 사용자에게 나가지 못한다.
 *
 * **왜 저장하는가.** 슬롯 결과(`saved_slots`)와 미션(`missions`)이 모두
 * `place_id`를 가리킨다. 뽑기만 하고 담지 않으면 **이어 붙일 곳이 없다.**
 * 나중에 결과를 다시 조회할 때 `content_id`로 실시간 세부 정보도 받아 온다.
 *
 * **여러 번 뽑혀도 안전하다.** `content_id`로 찾아 있으면 갱신한다. 온디맨드에서는
 * 인기 있는 장소가 반복해서 뽑히므로, 막지 않으면 같은 곳이 계속 쌓인다.
 *
 * ⚠️ **폐기된 배치(`PlaceSeeder`, 커밋 fe3ce39까지)에서 옮겨 온 로직이다.** 검증된 코드라 그대로 가져왔고,
 * 다만 **시·군 조회 캐시는 뺐다.** 배치는 1,241건을 한 번에 훑어 캐시가 필요했지만
...
public class PlaceUpserter {
     ...
     * api에서 가져온 장소 contentId로 DB 조회 후 Update or Insert
     *
     * 돌려주는 이유: 부르는 쪽(6-13)이 이 `Place`로 `saved_slots`를 만들고
     * 미션을 붙여야 한다. `void`면 다시 조회해야 한다.
     *
     ...
     * 좌표가 없으면 담지 않는다.
     *
     * `latitude`·`longitude`는 NOT NULL이라 넣으면 저장이 실패한다.
     *
     * **배치와 다르게 예외를 던진다.** 배치는 수천 건을 훑으며 한 건 때문에 멈추면 안 되므로
     * 조용히 건너뛰었다. 여기는 **이미 뽑힌 1건**이라 건너뛸 것이 없다.
     * 게다가 좌표 기반으로 조회해 받은 결과이므로 좌표가 없을 수 없다 —
     * 그런데도 여기 왔다면 **선정기가 걸러야 할 것을 놓친 우리 코드의 버그**다.
     */
    ...
    /**
     * 대표 이미지를 담는다. 슬롯 응답의 `thumbnailUrl`이 여기서 나온다.
     *
     * **지금 담아 두지 않으면 나중에 채울 방법이 없다.** 이미지 주소는 목록 응답에만 딸려 오므로,
     * 뒤늦게 필요해지면 그 장소를 다시 받아야 한다. 담는 비용은 이미 받은 값을 저장하는 것뿐이다.
     *
     * 없으면 행을 만들지 않는다. 빈 문자열로 만들면 `media_url`이 NOT NULL인데도
     ...
     *
     * **못 찾아도 장소는 담는다.** 여기서 멈추면 지역 표가 비어 있는 동안 슬롯 전체가 죽는다.
     * `places.city_id`를 nullable로 둔 이유가 이것이다.
     */
