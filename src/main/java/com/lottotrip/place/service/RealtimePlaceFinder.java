...
public class RealtimePlaceFinder {
     ...
     *
     * 숙소는 온보딩에서 따로 입력받으므로 여행지 후보가 아니다. 결정 13(전 종류 포괄)의
     * 부작용으로 섞여 있었고, 강릉 30km 후보 880건 중 190건(22%)이 숙박이었다.
     *
     * `cat2`가 `B0201`이거나 관광타입이 `32`면 뺀다. `cat2`는 비면 조용히 통과하고,
     * 관광타입만 보면 숙박 `cat2`가 늘었을 때 놓쳐서 둘 다 본다.
     *
     * ⚠️ `contentTypeId=32`로 요청하면 후보가 전부 걸러져 `NO_PLACE_FOUND`가 된다.
     */
