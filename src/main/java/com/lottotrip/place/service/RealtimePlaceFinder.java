 ...
 *
 * <p><b>슬롯 추첨의 전부가 여기서 일어난다.</b> 결정 12로 배치 적재가 없어지면서, 사용자가 무엇을
 * 보게 되는지는 이 클래스가 어떤 후보를 받아 오고 그중 무엇을 고르는가로 정해진다.
 *
 * <h2>왜 한 번만 부르는가</h2>
 * 후보 수를 먼저 알아보고 그다음 하나를 집어 오는 방법도 있지만(호출 2회), 실측해 보니
 * <b>응답 시간이 건수에 거의 영향받지 않는다</b> — 100건 0.37초 vs 880건 0.39초.
 * 시간의 대부분이 왕복 지연이라, <b>한 번에 다 받는 편이 두 번 묻는 것보다 오히려 빠르다.</b>
 * 게다가 일일 할당량을 절반만 쓴다.
 *
 * <p><b>페이지를 넘기지 않는다.</b> 2026-08-14에 페이지 순회가 끝나지 않아 할당량 1,000회를
 * 통째로 태운 적이 있다. 순회 자체를 하지 않으면 그 사고가 구조적으로 불가능해진다.
 *
 * <h2>⚠️ 한계 — 후보 1,000건까지만 닿는다</h2>
 * {@code numOfRows}의 API 상한이 1,000이다. 그보다 많으면 {@code arrange=E}(거리순) 덕에
 * <b>먼 곳부터 잘린다.</b> 서울 30km는 후보 4,181건 중 가까운 1,000건만 와서 실질 반경이
 * 6.4km가 된다. <b>수도권을 다루지 않기로 해 그대로 둔 선택이다</b>(결정 15).
...
public class RealtimePlaceFinder {
    ...
    /**
     * 한 번에 받아올 후보 수. <b>API 상한과 같은 값이다.</b>
     *
     * <p>더 작게 잡으면 반경 안에 있는데도 뽑힐 수 없는 장소가 생긴다.
     ...
     *
     * <p>{@code ThreadLocalRandom.current()}는 "지금 이 스레드의 난수"를 주는 메서드라,
     ...
     * 반경 안에서 장소 하나를 뽑는다.
     *
     * <p><b>비어 있을 수 있다.</b> 후보가 하나도 없다는 뜻이다. 여기서 예외를 던지지 않는 이유는,
     * 그렇게 하면 이 클래스가 <b>"후보 없음은 404다"라는 HTTP 사정까지 알게 되기</b> 때문이다.
