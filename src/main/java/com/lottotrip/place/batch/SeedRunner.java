 ...
 *
 * <p>⚠️ <b>예전에는 장소 적재도 함께 돌렸다.</b> 결정 12(온디맨드)로 배치 적재가 폐기되면서
 * {@code PlaceSeeder}가 사라져 <b>지역 시드만 남았다.</b> 장소는 이제 뽑힐 때마다
 * {@code PlaceUpserter}가 한 건씩 담는다.
 *
 * <p><b>지역 표는 여전히 필요하다.</b> {@code places.city_id}를 채우려면 {@code states}·{@code cities}가
 * 있어야 하는데, 이건 TourAPI에서 받아야 얻을 수 있다. 다만 호출이 2회로 끝나고
 * 지역 정보는 거의 바뀌지 않아 <b>사람이 필요할 때 한 번 돌리는</b> 방식 그대로 둔다.
 *
 ...
 *
 * <p><b>왜 기동 시 자동 실행이 아닌가.</b> 서버를 띄울 때마다 돌면 개발 중에 할당량이 조용히 깎이고,
...
public class SeedRunner implements CommandLineRunner {
     ...
     * 시도·시군구 표를 채운다.
     *
     * <p><b>슬롯을 돌리기 전에 한 번 돌려 두어야 한다.</b> 지역 표가 비어 있어도 슬롯은 동작하지만
     * ({@code city_id}가 nullable이다) 뽑힌 장소가 어느 시·군인지 알 수 없게 된다.
     */
    @Override
