...
public class CourseItem {
    ...
    /**
     * 이 항목을 담게 된 슬롯. (roadmap 7-6) - erd 변경사항
     *
     ...
     * 슬롯을 거치면 `saved_slots.mission_id` 한 개로 좁혀진다.
     *
     * NOT NULL인 이유: 담기 API가 `slotId`를 필수로 받으므로
     * 슬롯을 거치지 않은 항목은 만들어질 수 없다. nullable로 두면 DB가 그 있을 수 없는 상태를 허용함.
     */
    @ManyToOne(fetch = FetchType.LAZY)
    ...
    /** 코스 안에서의 순번. 담을 때 "그 코스의 마지막 순번 + 1"로 채운다.*/
