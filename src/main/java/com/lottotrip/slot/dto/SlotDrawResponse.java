 ...
 *
 * <p><b>Entity를 그대로 내보내지 않는 이유.</b> {@code Place}에는 {@code contentId}·
 * {@code modifiedTime}처럼 우리 사정에 쓰는 값이 함께 있다. 그대로 내보내면 필요 없는 것이 새어 나가고,
 * 반대로 <b>테이블 구조를 바꿀 때마다 API 응답이 따라 흔들린다.</b> 응답 모양을 따로 두면
 * DB는 DB대로, API는 API대로 바꿀 수 있다.
 *
 * @param slotId  {@code saved_slots.slot_id}. 7단계 코스 추가가 이 값을 참조한다
 * @param place   뽑힌 장소
 * @param mission 함께 제시할 미션. <b>없을 수 있다</b> — 생성까지 실패한 경우이며,
...
public record SlotDrawResponse(
    ...
    /**
     * @param category    한글 표시 이름("해변"). DB에는 {@code BEACH}로 저장돼 있다
     * @param distanceKm  기준 좌표로부터의 거리. 소수 첫째 자리까지만 담는다
     * @param thumbnailUrl 대표 이미지. <b>실측 채움률이 18%라 대개 null이다</b>
     */
