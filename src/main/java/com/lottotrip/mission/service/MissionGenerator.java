 ...
 *
 * **⛔ 생성 방식은 아직 정해지지 않았다**(2026-08-13 회의에서 보류, 다음 회의로 넘김).
 * LLM으로 문구를 만들지, 키워드·템플릿으로 조합할지가 갈린다. 그래서 **인터페이스만 먼저 두고**
 * 매칭·선택 로직({@link MissionMatcher})을 완성해 둔다. 회의 결과가 나오면
 * **구현체 하나만 갈아 끼우면 된다** — 4-3의 `SocialTokenVerifier`와 같은 구조다.
 *
 * ⚠️ **LLM으로 정해지면 호출 위치를 다시 봐야 한다.** 지금 구조는 draw 도중에 생성하므로,
...
public interface MissionGenerator {
     ...
     *
     * **요청한 개수를 다 못 채워도 된다.** 생성이 일부만 성공하거나 아예 실패할 수 있는데,
