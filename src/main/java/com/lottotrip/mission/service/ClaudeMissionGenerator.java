 ...
 *
 * 6-5에서 "구현체만 갈아 끼운다"를 전제로 인터페이스를 먼저 만들어 뒀고, 이제 그 자리를 채운다.
 * {@link MissionGenerator} 인터페이스와 `MissionMatcher`는 손대지 않았다 —
 * 4-3의 `SocialTokenVerifier`와 같은 구조다.
 *
 * **`@Primary`가 붙은 이유** — 이제 `MissionGenerator`를 구현한 빈이 둘이다
 * (이것과 {@link TemplateMissionGenerator}). 스프링은 둘 중 어느 것을 넣어야 할지 몰라
 * 주입 시점에 실패한다. `@Primary`는 "고민되면 이걸 써라"는 표시다.
 *
 * **템플릿 구현을 지우지 않은 이유** — 여기서 **폴백**으로 쓴다. 키가 없거나,
...
public class ClaudeMissionGenerator implements MissionGenerator {
     ...
     *
     * ⚠️ 잠정값이다(미확정 항목 "6-5 ③"). `reward_point`가 NOT NULL이라 정해 둔 것이고,
    ...
    /**
     * Claude에게 문구를 받아 미션으로 옮긴다. 실패하면 **빈 목록**을 돌려준다.
     *
     ...
     * 우리는 템플릿으로 간다"가 정확히 우리가 원하는 규칙**이다.
     */
     ...
     * 저장해도 되는 문구인지 본다.
     *
     * 모델이 준 값을 그대로 믿지 않는 지점이다. 제목이 비면 사용자에게 빈 미션이 보이고,
     * 100자를 넘으면 INSERT가 실패한다. **길면 잘라 붙이지 않고 버린다** —
