 ...
 * 그럼에도 최소 구현을 두는 이유는, 아무것도 만들지 않으면 **미션이 없는 장소가 뽑혔을 때
 * 응답이 비게 되어** 회의에서 "3개를 채운다"고 정한 취지가 무너지기 때문이다.
 *
 * **GPS 인증과 어긋나지 않게 "가서 하면 되는 것"만 담았다.** 8-1의 인증은 결국
...
public class TemplateMissionGenerator implements MissionGenerator {
     ...
     * 생성 미션의 보상 점수.
     *
     * ⚠️ **잠정값이다**(미확정 항목 "6-5 ③"). `reward_point`가 NOT NULL이라
     ...
     * `index`번째 미션의 제목.
     *
     * 요청이 템플릿 개수를 넘으면 처음으로 돌아가는데, 그대로 두면 제목이 그대로 겹친다.
     * 같은 제목이 여러 개면 사용자에게 같은 미션이 반복해 보이고, 나중에
     * `(place_id, title)` UNIQUE를 걸기로 하면 저장 자체가 실패한다.
