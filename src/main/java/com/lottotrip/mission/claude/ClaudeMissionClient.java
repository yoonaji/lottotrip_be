 ...
 *
 * <p><b>생성기({@link com.lottotrip.mission.service.ClaudeMissionGenerator})와 나눠 둔 이유</b> —
 * 이 클래스는 "바깥에 나갔다 오는" 일만 한다. 나눠 두면 생성기 쪽 테스트에서 이것만 가짜로
 * 바꿔 끼울 수 있어, <b>네트워크 없이 폴백과 변환 규칙을 검증</b>할 수 있다.
 * 4-4의 {@code KakaoTokenVerifier}가 API 호출만 맡는 것과 같은 구조다.
 *
 * <p><b>실패하면 예외를 그대로 올린다.</b> "실패를 어떻게 수습할지"(템플릿으로 내려갈지)는
...
public class ClaudeMissionClient {
    ...
    /**
     * SDK 클라이언트. <b>처음 쓸 때 만든다(지연 생성).</b>
     *
     * <p>기동 시점에 만들면 키가 비어 있는 개발 환경에서 서버가 아예 못 뜬다.
     * 키가 없으면 {@link #write}가 호출되지 않으므로 이 필드도 끝까지 비어 있는다.
     *
     * <p>{@code volatile}은 "여러 스레드가 이 필드를 볼 때 서로 최신 값을 보게 하라"는 표시다.
    ...
    /**
     * 이 장소에 쓸 미션 문구를 {@code count}개 요청한다.
     *
     * <p><b>구조화 출력(structured output)을 쓴다.</b> {@link MissionCopies} record로부터 SDK가
     * JSON 스키마를 만들어 함께 보내면, 모델은 그 형식의 JSON만 답한다. 자유 문장을 받아
     * 우리가 잘라 쓰면 모델이 형식을 조금만 바꿔도 파싱이 깨진다.
     *
     * @return 받은 문구들. 개수·내용은 <b>검증하지 않은 상태</b>이며, 거르는 것은 부르는 쪽의 몫이다
     ...
     * 모델에게 줄 지시문.
     *
     * <p><b>"도착하면 달성되는 것만"이라는 제약이 핵심이다.</b> 완료 판정은 GPS 위치 하나뿐인데
     ...
     *
     * <p><b>타임아웃을 반드시 건다.</b> SDK 기본값은 10분이라, 그대로 두면 Claude가 늦을 때
     * 슬롯 돌리기 응답이 10분간 멈춰 있는다. 짧게 잡고 넘기면 템플릿으로 내려가는 편이 낫다.
     *
     * <p>{@code synchronized}는 "이 블록은 한 번에 한 스레드만 지나간다"는 뜻이다.
     ...
     * 구조화 출력의 최상위가 <b>객체</b>여야 하기 때문이다(JSON 스키마 규칙).
     */
