 ...
 *
 * <p><b>Entity가 아니라 별도의 record인 이유</b> — 모델이 준 값은 아직 우리 것이 아니다.
 * 제목이 비어 있거나 컬럼 길이를 넘을 수 있어서, 그대로 {@code Mission}으로 받으면
 * 검사 없이 DB로 흘러간다. 여기서 한 번 받아 두고 {@code ClaudeMissionGenerator}가 걸러 옮긴다.
 *
 * <p>{@code @JsonPropertyDescription}은 <b>모델에게 이 필드가 무엇인지 알려 주는 설명</b>이다.
