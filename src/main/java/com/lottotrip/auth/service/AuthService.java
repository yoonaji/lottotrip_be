...
public class AuthService {
     ...
     *
     * `@Component`가 붙은 구현체가 새로 생기면 자동으로 이 목록에 포함된다.
     * 그래서 provider가 늘어도 이 클래스는 고치지 않는다 —
     * "기능 추가에는 열려 있고 수정에는 닫혀 있다"는 개방·폐쇄 원칙이 이런 모양이다.
     *
     * 참고: 생성자가 하나뿐이면 `@Autowired`를 생략해도 스프링이 알아서 주입한다.
     */
     ...
     * 진행하면 `NullPointerException`이 나 500으로 떨어지는데, 그것은 서버 잘못이 아니라
     * **요청이 잘못된 것**이므로 400이 맞다. (tour_api_erd.md 4-1 로그인)
     *
     * 애플은 개발자 계정 미보유로 구현체가 아직 없다(roadmap 4-4-3 보류).
     ...
     *
     * 사람을 찾는 기준은 **`provider` + `providerUserId`**다. 이메일로 찾지 않는다.
     * 이메일은 사용자가 바꿀 수 있고, 애플에서는 아예 오지 않을 수도 있기 때문이다.
     *
     * `@Transactional`은 "이 메서드 안의 DB 작업을 전부 성공시키거나 전부 되돌린다"는 뜻이다.
     * 신규 가입은 **회원 저장 + 소셜 계정 저장** 두 번의 INSERT인데, 둘 사이에서 실패하면
     ...
     *
     * 이 서비스의 리프레시 토큰은 **서버에 저장하지 않는다(stateless).** 서명이 맞고 만료되지
     * 않았으면 유효한 것으로 본다. 저장소가 필요 없어 단순하지만, **한 번 발급한 토큰을 중간에
     * 무효화할 수 없다**는 뜻이기도 하다. 그래서 로그아웃(4-7)은 앱이 토큰을 지우는 것에 의존한다.
     *
     * 리프레시 토큰 자체는 새로 발급하지 않는다. 그래야 갱신을 반복해도 원래 만료 시점이 유지된다.
     * 매번 새로 주면 사용자가 앱을 계속 쓰는 한 로그인 상태가 무한히 연장된다.
     *
     * `readOnly = true`는 "이 트랜잭션은 읽기만 한다"는 표시다. JPA가 변경 감지를 위한
     ...
     *
     * **서버가 할 일이 거의 없다.** 우리 토큰은 서버에 저장하지 않기로 했으므로(stateless),
     * "이 토큰은 이제 무효"라고 표시할 곳이 없다. 이미 발급된 토큰은 만료될 때까지 그대로 유효하다.
     * 실제 로그아웃은 **앱이 갖고 있는 토큰을 지우는 것**으로 이뤄진다.
     *
     * 그럼에도 이 API가 필요한 이유는 두 가지다. 앱 입장에서 "로그아웃 절차를 서버가 확인해 줬다"는
     * 신호가 있어야 하고, 서버는 **누가 언제 로그아웃했는지를 기록**할 수 있다.
     *
     * 토큰을 진짜로 무효화하려면 발급한 토큰을 서버가 들고 있어야 한다(저장소 또는 블랙리스트).
     ...
     *
     * 이름·이메일을 **반드시 이 시점에 저장**해야 한다. 애플·구글은 최초 1회만 주기 때문에
     ...
     *
     * `save()`를 부르지 않는 이유는, 트랜잭션 안에서 조회한 Entity는 JPA가 계속 지켜보고 있다가
     * **값이 바뀐 것을 스스로 발견해** UPDATE를 날려 주기 때문이다(변경 감지).
