...
public interface SocialAuthRepository extends JpaRepository<SocialAuth, Long> {
     ...
     *
     * 탈퇴에서 유일하게 행을 실제로 지우는 곳이다. 식별자·토큰이 개인정보이기도 하고,
     * 남겨 두면 같은 소셜 계정으로 로그인했을 때 탈퇴 계정이 되살아나기 때문이다
     * (로그인은 `provider` + `provider_user_id`로 사람을 찾는다). 지우면 재로그인은 신규 가입이 된다.
     *
     * ⚠️ 쓰기 동작이라 트랜잭션 안에서 불러야 한다.
     */
