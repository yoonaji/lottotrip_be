 ...
 *
 * <p>카카오는 구글·애플과 검증 방식이 다르다. 구글·애플은 받은 토큰이 JWT라서 <b>서명만 계산하면</b>
 * 진짜인지 알 수 있지만, 카카오가 주는 access token은 그냥 불투명한 문자열이라
 * <b>카카오에 직접 물어봐야</b> 한다. 그래서 여기만 외부 HTTP 호출이 들어간다.
 *
 * <p>"토큰이 유효한가"를 따로 묻는 API가 있는 게 아니라, 사용자 정보 조회가 성공하면 유효한 것이고
 * 401이 오면 유효하지 않은 것이다.
 */
@Slf4j
@Component
public class KakaoTokenVerifier extends SocialTokenVerifier {
     ...
     *
     * <p>응답 상태에 따라 에러를 나눈다. 무엇이 잘못됐는지에 따라 사용자가 할 일이 다르기 때문이다.
     * <ul>
     *   <li>4xx — <b>토큰이 잘못됐다.</b> 다시 로그인해야 한다 → {@code INVALID_PROVIDER_TOKEN}(401)</li>
     *   <li>5xx·연결 실패 — <b>카카오가 지금 이상하다.</b> 잠시 뒤 다시 시도하면 된다 → {@code SERVICE_UNAVAILABLE}(503)</li>
     * </ul>
     * 카카오 점검 중에 401을 돌려주면 사용자는 자기 계정이 문제인 줄 알고 로그인을 반복하게 된다.
     */
    @Override
