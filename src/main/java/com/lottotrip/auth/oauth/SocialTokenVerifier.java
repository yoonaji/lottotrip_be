 ...
 *
 * <p>provider마다 검증 방식이 다르다.
 * <ul>
 *   <li>카카오 — access token으로 카카오 사용자 정보 API를 호출한다 (외부 HTTP)</li>
 *   <li>구글·애플 — identity token(JWT)을 공개키(JWKS)로 서명 검증한다 (계산)</li>
 * </ul>
 *
 * <p>하지만 <b>앞뒤로 붙는 검사는 셋 다 같다.</b> "토큰이 비었나" → 검증 → "사용자 식별자가 나왔나".
 * 이 공통 순서를 부모가 {@link #verify(String)}에 고정하고, 가운데 한 칸({@link #doVerify})만
 * 자식이 채우게 하는 방식을 <b>템플릿 메서드 패턴</b>이라 한다.
 *
 * <p>이렇게 하는 이유는 두 가지다.
 * <ol>
 *   <li>같은 검사를 세 곳에 복사하지 않아도 된다. 나중에 검사를 하나 추가할 때 한 곳만 고치면 된다.</li>
 *   <li>자식이 검사를 <b>빠뜨릴 수 없다.</b> {@code verify}가 {@code final}이라 자식이 덮어쓸 수 없기 때문이다.</li>
 * </ol>
 */
public abstract class SocialTokenVerifier {
     ...
     *
     * <p>{@code final}을 붙여 자식이 이 흐름 자체를 바꾸지 못하게 막는다. 자식이 여기를 덮어쓰면
     ...
     *
     * <p>{@code protected}라서 바깥에서는 직접 호출할 수 없다. 검사를 건너뛴 채 이 메서드만
     * 부르는 길을 막아 두는 것이다. 반드시 {@link #verify(String)}를 거쳐야 한다.
     */
    protected abstract OAuthUserInfo doVerify(String token);

    /** 이 구현체가 담당하는 provider. {@code AuthService}가 구현체를 고를 때 쓴다. */
    public abstract ProviderType getType();

    /**
     * 빈 토큰은 외부 호출 전에 걸러낸다.
     *
     * <p>어차피 실패할 요청을 카카오·구글 서버까지 보내면 응답이 느려지고 쓸데없는 트래픽이 된다.
     */
     ...
     *
     * <p>{@code providerUserId}는 로그인이 사람을 찾는 유일한 기준이다(이메일로 찾지 않는다).
     * 이것이 비어 있으면 회원을 조회할 수도, 새로 만들 수도 없다. 그대로 진행하면
     * {@code provider_user_id}가 빈 문자열인 유령 회원이 생긴다.
     */
