 ...
 *
 * <p>공개키는 "누구나 봐도 되는 열쇠"다. 이것으로 서명을 <b>확인</b>할 수는 있지만
 * 서명을 <b>만들 수는</b> 없다. 만드는 쪽 열쇠(개인키)는 구글만 갖고 있으므로,
 * 서명이 맞으면 그 토큰은 구글이 발급한 것이 확실하다.
 *
 * @param keys 키가 여러 개인 이유는 <b>교체 중</b>이기 때문이다. 새 키로 바꾸는 동안에도
...
public record JwkSetResponse(List<Jwk> keys) {
    ...
    public record Jwk(String kid, String kty, String alg, String n, String e) {
         ...
         *
         * <p>RSA 공개키는 사실 <b>큰 정수 두 개</b>(n, e)가 전부다. JWKS는 이 숫자를
         * base64url이라는 방식으로 문자열에 담아 보낸다. 그래서 문자열 → 바이트 → 정수 → 키
         * 순서로 되돌린다.
         *
         * <p>{@code new BigInteger(1, bytes)}의 {@code 1}은 "이 숫자는 양수다"라는 뜻이다.
