 ...
 *
 * <p><b>왜 따로 필요한가.</b> 시큐리티가 막은 요청은 컨트롤러까지 가지 않는다.
 * 그래서 {@code @RestControllerAdvice}인 {@code GlobalExceptionHandler}가 손댈 수 없고,
 * 그대로 두면 스프링 기본 형식으로 응답이 나간다. 프론트 입장에서는 같은 서버가
 * 두 가지 에러 포맷을 내려주는 셈이라, 디코딩 코드를 두 벌 만들어야 한다.
 *
 * <p>그래서 이 지점에서 응답 본문을 직접 써서 공통 포맷({@code success}/{@code data}/{@code error})을
...
public class JwtAuthenticationEntryPoint implements AuthenticationEntryPoint {
     ...
     *
     * <p>패키지가 {@code tools.jackson}인 점에 주의. Spring Boot 4는 Jackson 3을 쓰는데,
     * Jackson 2({@code com.fasterxml.jackson})와 패키지 이름이 아예 다르다.
     * 습관대로 {@code com.fasterxml}을 import하면 컴파일은 되지만
