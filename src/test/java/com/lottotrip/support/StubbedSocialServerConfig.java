 ...
 * 이 가짜 통로를 쓰게 만든다. <b>검증기 자체는 진짜 코드가 그대로 돈다</b> — 바꾸는 것은
 * "바깥으로 나가는 길"뿐이다.
 *
 * <p>둘을 한 객체({@link Stub}) 안에서 함께 만드는 이유는 <b>순서</b> 때문이다. 검증기가 builder로
 * {@code build()}를 부르기 전에 가짜 연결이 꽂혀 있어야 한다. 따로 만들면 검증기가 먼저 생겨
 * 진짜 카카오로 요청이 나갈 수 있다.
 *
 * <p>원래 {@code AuthIntegrationTest} 안에 있던 것을 9-1에서 밖으로 뺐다. 전체 시나리오 테스트도
