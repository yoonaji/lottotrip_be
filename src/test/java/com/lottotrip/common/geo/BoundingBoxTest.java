 ...
 *
 * 이 사각형은 **1차 필터**다. Haversine은 삼각함수라 인덱스를 타지 못해, 바로 계산하면
 * 강원 전체를 매번 훑는다. 사각형으로 `idx_places_coordinate`를 태워 수십 건으로 줄인 뒤
 * 정확한 거리를 재는 편이 훨씬 싸다.
 *
 * **가장 중요한 성질: 사각형이 원을 완전히 감싸야 한다.** 조금이라도 작으면
 * 반경 안에 있는 장소가 1차 필터에서 탈락해 **영영 뽑히지 않는다.** 그 반대(원 밖이 조금
 * 딸려 오는 것)는 2차 Haversine이 걸러 주므로 문제가 되지 않는다. 즉 **넉넉한 쪽으로 틀려야 한다.**
 */
