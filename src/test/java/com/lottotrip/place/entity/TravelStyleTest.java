 ...
 *
 * ⚠️ **매핑 값 자체는 잠정이다.** 회의에서 확정되지 않아 실측 분포를 보고 임의로 정했다
 * (사용자 결정 2026-08-17 — "일단 임의로 매핑"). 아래 테스트는 **값이 맞다는 증명이 아니라
 * 지금 무엇으로 정해 두었는지의 기록**이다. 회의에서 값이 바뀌면 이 테스트도 함께 고친다.
 *
 * 여기서 지키는 것은 값이 아니라 **성질** 두 가지다.
 * - 한 분류가 두 스타일에 동시에 들어가지 않는다 (들어가면 "이 장소의 스타일"이 하나로 안 정해진다)
 * - 어느 스타일에도 없는 분류가 무엇인지 드러난다 (쟁점 ②)
 */
