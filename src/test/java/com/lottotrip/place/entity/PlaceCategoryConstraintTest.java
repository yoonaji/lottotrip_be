...
/**
 * `places.category`의 CHECK 제약이 enum과 어긋나지 않는지 확인한다. (roadmap 6-15, 결정 16)
 *
 * ## 이 테스트의 한계를 먼저 밝힌다
 * **2026-08-15에 겪은 사고를 이 테스트가 재현하지는 못한다.** 테스트용 DB는 매번 새로 만들어져
 * 제약이 **항상 최신 enum으로** 생성되기 때문이다. 사고는 **오래 살아 있는 DB**에서만 난다 —
 * 테이블은 옛 enum으로 만들어졌는데 코드만 새 값을 갖게 되는 상황이다.
 *
 * 그래도 남겨 두는 이유는 **Hibernate가 이 제약을 만든다는 사실을 코드에 박아 두기 위해서**다.
 * 제약이 있다는 걸 모르면, enum에 값을 더할 때 DB도 함께 손봐야 한다는 생각을 못 한다.
 *
 * **근본 해결은 마이그레이션 도구(Flyway)다.** 스키마 변경을 코드와 함께 버전 관리해야
