 ...
 * `cat1`~`cat3`를 섞어 11종으로 나눴는데, 지금은 **`cat2` 하나만** 본다.
 *
 * **왜 바꿨나.** 여행 스타일 6종(휴식·맛집·감상·체험·활동·탐험)으로 후보를 거르려는데
 * 기존 분류로는 **체험·활동·탐험이 전부 `LEISURE` 하나로 뭉쳤다.**
 * `cat2`에는 "휴양관광지"·"체험관광지"가 이름 그대로 있어 그대로 쓸 수 있다.
 */
