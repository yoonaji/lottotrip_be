 ...
 *
 * <p><b>확정된 규칙:</b> 장소에 등록된 미션이 <b>3개</b> 미만이면 생성해 채운 뒤,
 * 확보된 전체에서 랜덤 1개를 제시한다. 미션이 하나도 없는 장소를 뽑아도 응답이 비지 않게 하려는 것이다.
 *
 * <p><b>생성 방식(LLM vs 키워드)은 보류다.</b> 그래서 {@link MissionGenerator}를 인터페이스로 두고
 * 여기서는 <b>매칭·선택 로직만</b> 검증한다. 테스트는 가짜 생성기를 끼워 "몇 개를 요청했는지"와
...
class MissionMatcherTest extends PostgresContainerSupport {
    ...
    /**
     * 시드를 고정한 난수기. <b>인스턴스를 하나 두고 계속 쓴다.</b>
     *
     * <p>{@code () -> new Random(42)}처럼 공급자 안에서 만들면 부를 때마다 같은 시드의
     * 새 난수기가 생겨 <b>항상 같은 첫 값</b>이 나온다. 난수가 고정되는 게 아니라 한 값에 박힌다.
     */
